...
typedef struct {
    // Size is calculated by looping through both
    // escape maps. Max size is 10 but lets add 2
    // just to be safe.
...
static NSString * stringFromEscapeSequence (char escapeSequence[]) {
    return [[NSString alloc] initWithCString:escapeSequence encoding:NSUTF8StringEncoding];
}
...
//		_GTMDevLog("Unable to allocate buffer or data2");
	...
	for (NSUInteger i = 0; i < length; ++i) {
		...
		if (val || (escapeUnicode && buffer[i] > 127)) {
			...
			if (val) {
				[finalString appendString:stringFromEscapeSequence(val->escapeSequence)];
			}
			else {
		...
		if (length > 3 && length < 11) {
			if ([escapeString characterAtIndex:1] == '#') {
				...
				for (unsigned i = 0; i < sizeof(gAsciiHTMLEscapeMap) / sizeof(HTMLEscapeMap); ++i) {
					if ([escapeString isEqualToString:stringFromEscapeSequence(gAsciiHTMLEscapeMap[i].escapeSequence])) {
